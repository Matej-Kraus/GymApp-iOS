  ...
  return (
    <View className="flex-1 rounded-2xl bg-card border border-white/10 p-4 items-center">
      <View className="flex-row items-baseline">
        <Text className={cn('font-display text-2xl font-bold', accent ? 'text-accent' : 'text-white')}>{value}</Text>
...
export default function WorkoutSummary() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-bg" edges={['top']}>
      <View className="px-5 pt-8 pb-6 items-center gap-2">
        <Text className="text-5xl">💪</Text>
        <Text className="font-display text-2xl font-bold text-accent">Trénink dokončen!</Text>
      ...
      <View className="px-4 gap-4 flex-1">
        <View className="flex-row gap-3">
          <StatCard value={volume.toLocaleString('cs-CZ')} unit="kg" label="Objem" accent />
          <StatCard value={String(sets)} label="Sérií" />
        </View>
        <View className="flex-row gap-3">
          <StatCard value={String(exerciseCount)} label="Cviků" />
          {volumeDelta !== null ? (
            <StatCard
            ...
            <StatCard value="—" label="vs. minule" />
          )}
        </View>

        {hasPR && (
          <View className="flex-row items-center gap-3 rounded-2xl bg-accent/10 border border-accent/30 px-4 py-3">
